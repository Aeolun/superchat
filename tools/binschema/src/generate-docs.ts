...
import { ProtocolSchema, validateProtocolSchema } from './schema/protocol-schema.js';
...
async function main() {
  ...
  const protocolSchema = JSON.parse(protocolSchemaRaw) as ProtocolSchema;
  ...
  const binarySchema = JSON.parse(typesSchemaRaw) as BinarySchema;
